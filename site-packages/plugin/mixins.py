...
class TableListViewMixin(object):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context["fields"] = [field.name for field in self.model._meta.get_fields() if not field.name in [self.model._meta.pk.attname,] and not isinstance(field, models.ManyToOneRel) and not field.name in self.fields_exclude] \
...
class TableDetailViewMixin(ShortcutURLMixin):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if not self.fieldsets:
            context["fields"] = [field for field in self.model._meta.get_fields() if not field.name in self.fields_exclude and not field.name in [self.model._meta.pk.attname,] and not isinstance(field, models.ManyToOneRel)] \
                    if not self.fields else self.fields
        # lookup_field
        # _get_non_gfk_field
...
class DeleteViewMixin(ShortcutURLMixin):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        if not context.get("back_url",None) and self.get_object().get_absolute_url_list():
            context["back_url"] = self.get_object().get_absolute_url_list()
        return context
