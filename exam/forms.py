...
class ExamLibItemForm(ModelForm):
    ...
    source = CharField(label="Source", widget=Textarea(attrs={'cols': 10, 'rows': 1}), required=False) 
    contributor = CharField(label="Contributor", widget=Textarea(attrs={'cols': 10, 'rows': 1}), required=False) 
...
class ExamItemForm(ModelForm):
    ...
    def clean_score_result(self):
        ...
        return self.cleaned_data.get("score_result")
